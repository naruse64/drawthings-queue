...
| `model.backfilled` | **後追いで埋めた値のときだけ `true`。** 生成時に記録された値には付かない |
...
`model` を記録し始めたのは途中からで、それ以前の記録には**あとから値を埋めた**。
その分には `model.backfilled: true` が付く。

```json
"model": { "file": "example_base_model_f16.ckpt", "backfilled": true }
```

**生成時に記録された値には、このキーは付かない**（`false` ではなく不在）。
`model.get("backfilled")` が真かどうかで判定できる。

後追い値は**当時のモデルを裏付ける記録が残っていない**ため、設定が変わって
いないという申告に基づく。生成時に観測した値とは根拠の強さが違うので、
モデルを変数として分析する場合はこの印を見ること。

**`sha256` にこの印は付かない。** 後追いで計算した分もあるが、これは
「そのファイルのハッシュ」という観測であって過去についての推測ではない。
`model` だけが推測にあたる。
...
持たなかった（アプリ由来 224枚、手動生成 33枚、記録を失った dtq 出力 1枚）。
**このうち 257枚は別フォルダへ移したが、混在しない仕組みがあるわけではないので
再発しうる。**

残る1枚は dtq が生成したものなので移していない。そのため
**画像の枚数と `result.json` の件数は一致しない**（現在 1996枚に対し 1995件）。
枚数の突き合わせを整合性チェックに使わないこと。
