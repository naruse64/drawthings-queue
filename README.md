...
`bin/dtq-status` が画像原本の枚数・容量とディスクの空きを出す。

```
画像原本 : 1595 枚 / 1.7G（/Users/kazuya/DrawThings。保持期間の対象外）
ディスク : 空き 86Gi / 全体 460Gi
```

30日で自動削除されるのは iCloud 側の `thumbs/` と `results/` だけで、**画像原本は
貯まり続ける**。生成物の自動削除は取り返しがつかないため、消す仕組みは持たない。
容量を見て判断する。
