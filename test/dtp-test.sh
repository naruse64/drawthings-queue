...
f=$(scene en '主題: a woman in her twenties\n場所: a seaside promenade\nカメラ: cowboy shot\n')
check "英語は . で区切る" \
  "$("$DTP" compose "$f" 2>/dev/null)" "a seaside promenade. a woman in her twenties. cowboy shot."

f=$(scene enperiod '主題: a woman.\n場所: a beach.\n')
check "既に . があれば足さず間だけ空ける" \
  "$("$DTP" compose "$f" 2>/dev/null)" "a beach. a woman."

f=$(scene mixed '主題: a woman in her twenties\n場所: 夕方の喫茶店\n')
check "和英が混ざっても壊れない" \
  "$("$DTP" compose "$f" 2>/dev/null)" "夕方の喫茶店。a woman in her twenties."
